#!/usr/bin/env Rscript
# TCGA Python数据 - LASSO Biomarker筛选

cat("=== TCGA Python数据 - LASSO Biomarker筛选 ===\n\n")
...
dds <- readRDS(file.path(save_dir, "tcga_python_dds_analyzed.rds"))
deseq_res <- readRDS(file.path(save_dir, "tcga_python_deseq_results.rds"))
...
sig_genes_df <- read.csv(file.path(save_dir, "tcga_python_sig_genes.csv"), row.names = 1)
...
nfolds <- min(8, floor(n_per_class / 2))
...
# ROC曲线
pdf(file.path(save_dir, "tcga_python_lasso_roc_curve.pdf"), width = 8, height = 6)
print(plot_roc_curve(cv_fit, X, y, lambda = lambda_choice))
dev.off()
cat("   ✓ ROC曲线\n")

# 系数路径
pdf(file.path(save_dir, "tcga_python_lasso_coefficient_path.pdf"), width = 12, height = 6)
print(plot_coefficient_path(cv_fit))
dev.off()
cat("   ✓ 系数路径\n")

# 交叉验证曲线
pdf(file.path(save_dir, "tcga_python_lasso_cv_curve.pdf"), width = 8, height = 6)
...
if (biomarkers$n_features >= 2) {
  tryCatch({
    plot_biomarker_heatmap(
      ...
      output_file = file.path(save_dir, "tcga_python_lasso_biomarker_heatmap.pdf")
    )
    cat("   ✓ Biomarker热图\n")
  }, error = function(e) {
    cat("   ⚠️ 热图生成失败\n")
  ...
  if (biomarkers$n_features >= 3) {
    pdf(file.path(save_dir, "tcga_python_lasso_biomarker_boxplot.pdf"), width = 14, height = 6)
...
write.csv(
  importance,
  file.path(save_dir, "tcga_python_lasso_biomarker_importance.csv"),
  row.names = FALSE
)
...
write.csv(
  eval_summary,
  file.path(save_dir, "tcga_python_lasso_evaluation_summary.csv"),
  row.names = FALSE
)

# 保存LASSO模型
saveRDS(cv_fit, file.path(save_dir, "tcga_python_lasso_cv_fit.rds"))
saveRDS(biomarkers, file.path(save_dir, "tcga_python_lasso_biomarkers.rds"))

cat("   ✓ 所有结果已保存\n\n")

cat("=== TCGA LASSO分析完成 ===\n")
cat("\n📊 最终总结:\n")
cat("  - 数据来源: 真实TCGA-COAD数据（通过Python GDC API下载）\n")
