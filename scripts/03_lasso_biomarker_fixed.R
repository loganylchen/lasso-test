#!/usr/bin/env Rscript
# CRC真实数据分析 - Step 3: LASSO Biomarker筛选（修复版）
...
sig_genes_df <- read.csv(file.path(save_dir, "crc_sig_genes.csv"), row.names = 1)
...
if (biomarkers_1se$n_features >= 5) {
  ...
  # 如果还是太少，手动选择top 10差异基因
  cat("   ⚠️ LASSO筛选biomarker过少，手动选择top 10差异基因\n")
  top10_genes <- rownames(sig_genes_df)[1:10]
  biomarkers <- list(
    selected_features = top10_genes,
    coefficients = rep(1, 10),
    n_features = 10,
...
if (biomarkers$n_features >= 2) {
  tryCatch({
    ...
    cat("   ⚠️ 热图生成失败:", conditionMessage(e), "\n")
  })
